...
static void flowdeckTask(void *param)
{
  ...
  while(1) {
    ...
    if (abs(accpx) < OULIER_LIMIT && abs(accpy) < OULIER_LIMIT) {
      ...
      log_dx=flowData.dpixelx;//tom logging
      log_dy=flowData.dpixely;//tom logging
      ...
      log_dx=(float)accpx; //tom logging
      log_dy=(float)accpy; //tom logging
#endif
...
LOG_ADD(LOG_FLOAT, deltaX, &log_dx )//LOG_INT16  &currentMotion.deltaX)
LOG_ADD(LOG_FLOAT, deltaY, &log_dy )// &currentMotion.deltaY)
